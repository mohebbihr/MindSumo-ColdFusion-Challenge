        ...
        <script>
            (function($) {
                $(document).ready(function() {
                	$("##portaddlist:last").hide();                	                
                	
                	$('[class=toggle]').each(function(){$(this).toggle();});
                	
                    $("##portlist").dynamiclist();                                        
                    
                    // submit skillform data
                    $("##portform").submit(function(event) {
                        event.preventDefault();                          
                        // find a replace , with ~ in all 
                        $('[id=portsummary],[id=porttitle]').each(function(index, obj){
                        	str = $(this).val();						    
						    str = str.replace(new RegExp(',', 'g'), '~');
						    $(this).val(str);						    
						});                                             
                        // submit data to server                        
                        ColdFusion.Ajax.submitForm('portform', 'serverscripts/portscript.cfm', callback, errorHandler);                        
